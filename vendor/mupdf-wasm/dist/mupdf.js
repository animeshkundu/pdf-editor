...
export class Buffer extends Userdata {
	...
	writeBuffer(other) {
		withBuffer(other, pointer => libmupdf._wasm_append_buffer(this.pointer, pointer));
	}
	asUint8Array() {
		let data = libmupdf._wasm_buffer_get_data(this.pointer);
...
export class ColorSpace extends Userdata {
	constructor(from, name) {
		if (typeof from === "number") {
			...
			super(withBuffer(from, pointer => libmupdf._wasm_new_icc_colorspace(STRING_OPT(name), pointer)));
		}
	}
	getName() {
...
export class Font extends Userdata {
	constructor(name_or_pointer, data, subfont = 0) {
		...
		if (typeof name_or_pointer === "number") {
			...
			if (data)
				pointer = withBuffer(data, buffer => libmupdf._wasm_new_font_from_buffer(STRING(name_or_pointer), buffer, subfont));
...
export class Image extends Userdata {
	constructor(data, mask) {
		...
		if (typeof data === "number") {
			...
			pointer = withBuffer(data, buffer => libmupdf._wasm_new_image_from_buffer(buffer));
		}
		super(pointer);
	}
	getWidth() {
...
export class PDFFontDescriptor extends Userdata {
	constructor(pointer) {
...
PDFFontDescriptor._drop = libmupdf._wasm_pdf_drop_font;
...
export class DocumentWriter extends Userdata {
	constructor(buffer, format, options) {
		super(libmupdf._wasm_new_document_writer_with_buffer(buffer.pointer, STRING(format), STRING2(options)));
	}
	beginPage(mediabox) {
		checkRect(mediabox);
...
export class PDFDocument extends Document {
	...
	addObject(obj) {
		return this._withPDFObject(obj, pointer => this._fromPDFObjectNew(libmupdf._wasm_pdf_add_object(this.pointer, pointer)));
	}
	addStream(buf, obj) {
		return this._withPDFObject(obj, pointer => withBuffer(buf, buffer => this._fromPDFObjectNew(libmupdf._wasm_pdf_add_stream(this.pointer, buffer, pointer, 0))));
	}
	addRawStream(buf, obj) {
		return this._withPDFObject(obj, pointer => withBuffer(buf, buffer => this._fromPDFObjectNew(libmupdf._wasm_pdf_add_stream(this.pointer, buffer, pointer, 1))));
	}
	newGraftMap() {
	...
	addPage(mediabox, rotate, resources, contents) {
		...
		return this._withPDFObject(resources, pointer => withBuffer(contents, buffer => this._fromPDFObjectNew(libmupdf._wasm_pdf_add_page(this.pointer, RECT(mediabox), rotate, pointer, buffer))));
	}
	insertPage(at, obj) {
		checkType(at, "number");
		this._withPDFObject(obj, pointer => libmupdf._wasm_pdf_insert_page(this.pointer, at, pointer));
	}
	deletePage(at) {
		checkType(at, "number");
	...
	addEmbeddedFile(filename, mimetype, contents, created, modified, checksum = false) {
		...
		return withBuffer(contents, buffer => this._fromPDFObjectNew(libmupdf._wasm_pdf_add_embedded_file(this.pointer, STRING(filename), STRING2(mimetype), buffer, created.getTime() / 1000 | 0, modified.getTime() / 1000 | 0, checksum)));
	}
	...
	resetForm(fields, exclude) {
		this._withPDFObject(fields, pointer => libmupdf._wasm_pdf_reset_form(this.pointer, pointer, Number(exclude)));
	}
}
...
const PDF_OPERATOR_ARITY = new Map([
	...[
		"w", "j", "J", "M", "d", "i", "gs_ca", "gs_CA",
		"Tc", "Tw", "Tz", "TL", "Tf", "Tr", "Ts", "G", "g",
		"gs_OP", "gs_op", "gs_OPM",
	].map(operator => [operator, 1]),
	...["m", "l", "Td", "TD", "\"", "d0"].map(operator => [operator, 2]),
	...["RG", "rg"].map(operator => [operator, 3]),
	...["v", "y", "re", "K", "k"].map(operator => [operator, 4]),
	...["cm", "c", "Tm", "d1"].map(operator => [operator, 6]),
]);
export class PDFProcessorTrace extends Userdata {
	...
	getRecords() {
		...
		for (let index = 0; index < count; ++index) {
			...
			let payload = bytes.slice(payloadOffset + relativePayloadOffset, payloadOffset + relativePayloadOffset + recordPayloadLength);
			const handles = [];
			for (let slot = 0; slot < 3; ++slot) {
				...
				let value = null;
				if (handle && kind === 1)
					value = new PDFFontDescriptor(handle);
				else if (handle && kind === 2)
					value = this._doc._fromPDFObjectKeep(handle);
				else if (handle && kind === 3)
					value = new Image(handle);
				else if (handle && kind === 4)
					value = new ColorSpace(libmupdf._wasm_keep_colorspace(handle));
				else if (handle && kind === 6)
					value = new Shade(libmupdf._wasm_keep_shade(handle));
				if (value) {
					handles.push(value);
					this._ownedHandles.push(value);
				}
			}
			let operands = Array.from({ length: PDF_OPERATOR_ARITY.get(operator) ?? 0 }, (_, operand) => view.getFloat32(base + 36 + operand * 4, true));
			let name;
			if (PDF_COLOR_OPERATORS.has(operator)) {
				...
				operands = Array.from({ length: payload.length / 4 }, (_, operand) => colors.getFloat32(operand * 4, true));
			}
				...
				const colors = new DataView(payload.buffer, payload.byteOffset + colorOffset, payload.byteLength - colorOffset);
				operands = Array.from({ length: colors.byteLength / 4 }, (_, operand) => colors.getFloat32(operand * 4, true));
			}
				...
				operands = [
					...
					...Array.from({ length: colors.byteLength / 4 }, (_, operand) => colors.getFloat32(operand * 4, true)),
				];
			}
...
PDFProcessorTrace._drop = libmupdf._wasm_pdf_drop_processor_trace;
export class PDFPage extends Page {
	...
	processContents() {
		return new PDFProcessorTrace(this._doc, libmupdf._wasm_pdf_process_page_contents(this.pointer));
	}
	filterContents(options = {}) {
		const flags = ((options.recurse ?? true) ? 1 : 0) |
			((options.instanceForms ?? false) ? 2 : 0) |
			((options.ascii ?? false) ? 4 : 0) |
			((options.noUpdate ?? false) ? 8 : 0) |
			((options.newlines ?? true) ? 16 : 0);
		libmupdf._wasm_pdf_filter_page_contents(this.pointer, flags);
	}
	setPageBox(box, rect) {
		let box_ix = ENUM(box, Page.BOXES);
		checkRect(rect);
...
export class PDFObject extends Userdata {
	...
	writeObject(obj) {
		...
		this._doc._withPDFObject(obj, pointer => libmupdf._wasm_pdf_update_object(this._doc.pointer, this.asIndirect(), pointer));
	}
	writeStream(buf) {
		if (!this.isIndirect())
			throw new TypeError("can only call PDFObject.writeStream on an indirect reference");
		withBuffer(buf, buffer => libmupdf._wasm_pdf_update_stream(this._doc.pointer, this.pointer, buffer, 0));
	}
	writeRawStream(buf) {
		if (!this.isIndirect())
			throw new TypeError("can only call PDFObject.writeRawStream on an indirect reference");
		withBuffer(buf, buffer => libmupdf._wasm_pdf_update_stream(this._doc.pointer, this.pointer, buffer, 1));
	}
	resolve() {
	...
	_pushValue(value) {
		this._doc._withPDFObject(value, pointer => libmupdf._wasm_pdf_array_push(this.pointer, pointer));
	}
	delete(key) {
		if (typeof key === "number")
...
export class PDFAnnotation extends Userdata {
	...
	setFileSpec(fs) {
		return this._doc._withPDFObject(fs, pointer => libmupdf._wasm_pdf_set_annot_filespec(this.pointer, pointer));
	}
	getQuadPoints() {
	...
	setAppearance(appearance, state, transform, bbox, resources, contents) {
		checkMatrix(transform);
		checkRect(bbox);
		this._doc._withPDFObject(resources, pointer => withBuffer(contents, buffer => libmupdf._wasm_pdf_set_annot_appearance(this.pointer, STRING_OPT(appearance), STRING2_OPT(state), MATRIX(transform), RECT(bbox), pointer, buffer)));
	}

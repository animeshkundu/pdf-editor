...
| pdf.js                      | 6.1.200 | npm `pdfjs-dist`                  | Apache-2.0   | Independent PDF parser and 144 dpi renderer  |
| `@napi-rs/canvas`           |   1.0.2 | npm                               | MIT          | Native canvas used only by the pdf.js oracle |
| Terser                      |  5.39.2 | npm `terser`                      | BSD-2-Clause | Reproducible minification of the WASM loader |
...
| ----------------------------------------- | -------------------- | ---------------- | ----------------------------------------------------------------------------------------------- |
| Emscripten (emsdk) 4.0.8                  | emscripten-core      | MIT / NCSA       | Builds the forked MuPDF WASM ([ADR 0006](adr/0006-three-toolchain-build-and-committed-wasm.md)) |
...
| **qpdf 12.3.2**                           | qpdf/qpdf            | Apache-2.0       | **Independent structural validator**, pinned archive SHA-256 in `scripts/setup-qpdf.mjs`        |
